...
export default function Registration() {
  ...
  const [formData, setFormData] = useState({
    ...
    members: [
      { name: '', email: '', phone: '', college: '', year: '', branch: '', gender: '', otherGender: '' },
      { name: '', email: '', phone: '', college: '', year: '', branch: '', gender: '', otherGender: '' },
      { name: '', email: '', phone: '', college: '', year: '', branch: '', gender: '', otherGender: '' },
      { name: '', email: '', phone: '', college: '', year: '', branch: '', gender: '', otherGender: '' },
      { name: '', email: '', phone: '', college: '', year: '', branch: '', gender: '', otherGender: '' }
  ...
  const isTeamLeaderComplete = formData.teamLeader.name && 
  ...
  const isAtLeastOneMemberComplete = formData.members.some(member => 
  ...
  const isFormComplete = isTeamInfoComplete && isTeamLeaderComplete && isAtLeastOneMemberComplete;
  ...
  const toggleSection = (section: number) => {
    if (section === 1 || (section === 2 && canShowTeamLeader) || (section === 3 && canShowTeamMembers)) {
  ...
  useEffect(() => {
    if (user && !formData.teamLeader.name && !formData.teamLeader.email && !existingTeam) {
      setFormData(prev => ({
        ...prev,
        teamLeader: {
          ...prev.teamLeader,
          ...
          <div className="max-w-4xl mx-auto px-6 py-20">
            ...
            >
              <div className="text-center mb-8">
                <h1 className="text-4xl font-display text-heading mb-4">Team Registration Complete</h1>
                <p className="text-subheading font-body">Your team has been successfully registered!</p>
              </div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-text bg-gray-800/30 border border-gray-600 rounded-lg px-4 py-3">{existingTeam.teamName}</p>
                  </div>
                  <div>
                    <label className="block text-subheading text-sm font-medium mb-2">Status</label>
                    <p className="text-text bg-gray-800/30 border border-gray-600 rounded-lg px-4 py-3 capitalize">{existingTeam.status}</p>
                  </div>
                </div>
                
                <div className="mt-8 text-center">
                  <p className="text-text font-body">For any changes or queries, please contact the administrators.</p>
                </div>
              </div>
            </motion.div>
          </div>
    ...
    <ProtectedRoute>
      ...
      <section className="pt-32 pb-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
              Register your team for the Smart India Hackathon 2025 Internal Round.
          </motion.p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-medium text-gray-400 tracking-wide">Registration Progress</span>
              <span className="text-sm font-medium text-subheading tracking-wide">
                Step {isTeamInfoComplete ? (isTeamLeaderComplete ? '3' : '2') : '1'} of 3
              </span>
            </div>
            <div className="w-full bg-gray-800 rounded-full h-2">
              <motion.div
                ...
                initial={{ width: "0%" }}
                animate={{
                  width: isTeamInfoComplete ? (isTeamLeaderComplete ? "100%" : "66%") : "33%"
                }}
                transition={{ duration: 0.5, ease: "easeOut" }}
              />
            </div>
            <div className="flex justify-between mt-2 text-xs text-gray-500">
              <span className={isTeamInfoComplete ? 'text-heading' : ''}>Team Info</span>
              <span className={isTeamLeaderComplete ? 'text-heading' : ''}>Team Leader</span>
              <span className={canShowTeamMembers ? 'text-heading' : ''}>Team Members</span>
            </div>
          </motion.div>
          ...
          >
            ...
            <AccordionSection
              ...
              status={!canShowTeamLeader ? 'locked' : isTeamLeaderComplete ? 'completed' : 'required'}
            >
              <TeamLeader 
                teamLeader={formData.teamLeader}
                onInputChange={handleInputChange}
                isDataFromAuth={!!(user && (formData.teamLeader.name || formData.teamLeader.email))}
              />
            </AccordionSection>
