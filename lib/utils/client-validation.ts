// Client-safe validation utilities (no server-side model imports)
...
/**
 * Team member interface (client-side version)
 */
...
/**
 * Team form data interface (full data)
 */
...
/**
 * Simplified form data for duplicate checking
 */
...
/**
 * Validate team registration data (client-side version)
 */
...
/**
 * Client-side duplicate validation (no database queries)
 */
export const validateNoDuplicates = (
  formData: DuplicateCheckData
): {
...
/**
 * Admin registration validation (client-side version)
 */
...
export function validateAdminRegistration(data: AdminRegistrationData): {
  ...
  if (!data.adminSecretKey || data.adminSecretKey.trim().length === 0) {
